		...
		static void ProcessBuiltinChip(SimChip chip)
		{
			switch (chip.ChipType)
			{
				...
				case ChipType.Merge_1To16Bit:
				{
					...
					uint stateP = chip.InputPins[0].State & PinState.SingleBitMask; // msb
				...
				{
					uint redPin = chip.InputPins[0].State;
					uint greenPin = chip.InputPins[1].State;
					uint bluePin = chip.InputPins[2].State;

					uint data = (uint)(PinState.GetBitStates(redPin) | (PinState.GetBitStates(greenPin) << 4) | (PinState.GetBitStates(bluePin) << 8));
					chip.InternalState[0] = data;
